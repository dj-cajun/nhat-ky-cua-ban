...
export const ANON_BODY_MAX = 300;
export const ANON_PREVIEW_LIMIT = 3;
export const ANON_PAGE_LIMIT = 20;

export function validateAnonymousPostBody(body: string): void {
  const trim = body.trim();
  if (trim.length < 1 || trim.length > ANON_BODY_MAX) {
...
/** Relative time without second-level precision (anti-timing correlation). */
export function formatAnonymousRelativeTime(iso: string, now = Date.now()): string {
