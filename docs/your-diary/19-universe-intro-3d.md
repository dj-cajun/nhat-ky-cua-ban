# 19 — 내 우주 인트로 영상 → 2D 핸드오프

> **상태**: 현행  
> **최종 갱신**: 2026-07-24  
> **영상**: 사용자가 제작 · 앱은 슬롯 + 크로스페이드 + 2D 홈만 담당
...
인트로 영상 (공통)
→ 마지막 프레임 = 중앙 금빛 구체
→ 영상 opacity 1→0 / 2D opacity 0→1 (크로스페이드)
→ 프로필 페이드업 (사용자별)
→ 서클이 파스텔 궤도 다이어그램으로 등장
→ 중앙 구체 탭 → 파스텔 미니홈피 (/diary/[userId])
→ 서클 노드 탭 → 서클 상세 (이후 2D)
```

용량을 위해 **R3F/three/expo-gl은 넣지 않는다**. 내 우주 홈도 2D 파스텔 다이어그램.
...
**기본 영상**이 레포에 포함되어 있다. 직접 만든 영상으로 교체해도 된다. 앱은 슬롯 + 크로스페이드 + 2D 홈만 담당.
...
라이브 2D는 cover 기하에 `liveOpticalScale`(~0.88)을 곱해 **영상 소프트 림**과 맞춘 뒤,  
`homeDiameterRatio`(뷰포트 너비 ~13%)로 **천천히 줄어드는** 정착 애니(~2.4s)를 재생한다.  
정착 후 프렌즈 서클은 작은 중앙 원을 **파스텔 궤도 다이어그램**으로 크게 감싼다.
...
- 우주 홈: **2D** Reanimated + 파스텔 궤도 다이어그램 (`FallbackUniverse`)
- 전환: Reanimated opacity 크로스페이드 → settle-shrink
- ~~R3F / three / expo-gl~~ — **제거** (설치 용량). 필요 시 나중에 옵션으로만 재도입.
