...
- **AI 고해상도 초안은 톤·구도 참고용**. 구현 투입 전 **32~64px 타일 그리드**로 저해상도화·팔레트 고정 (용량 최소화).  
- 책상 상판 나무결: **가로** 결 (세로 줄무늬 금지 · 레퍼런스와 통일)

창 vs 코르크 (에셋이 달라도 `desk_bg` 자리부터 다르게):

| | 자리(desk_bg) | 얹는 레이어 |
|--|---------------|-------------|
| 창문 | 왼쪽 · **얇은** 창틀 홈 | 유리·반사·하늘 (`window_*`) |
| 코르크 | 오른쪽 · **코르크색 면** 홈 (동일 검은 액자 두 개 금지) | 오돌토돌 코르크 + 핀 슬롯 (`corkboard`) |
...
| `cork_empty` | `cork_empty.png` | 빈 폴라로이드(불투명 크림) · corkboard 구멍 **뒤**에 깔기 | 1 |
| `photo_frame` | `photo_frame.png` | 나무 액자 · **가운데 알파 구멍** · 대표 사진 뒤 합성 | 1 |
| `lamp_glow` | `lamp_glow.png` | 앰버 광 디더 글로우 (lamp 아래/위 오버레이) | 1 |
...
D3b 전체 연결 ✅ 캘린더 · 친구 앨범(읽기) · 저장 후 week 갱신 · e2 패리티
...
**연결**: 메모→편집→저장→desk+week · 코르크→앨범→저장→cork · 친구 미니앨범→읽기 전용 앨범 · 주간 헤더→캘린더.
