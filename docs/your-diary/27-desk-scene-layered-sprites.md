...
[  DeskScene  3:2 위젯 — 창|보드 / 책상 소품  ]   ← NEW (10자+사진첩 대체)
[주간 캘린더]   ← 유지 (주에 10자 미리보기는 보조로 남을 수 있음)
[써클·방명록·자유 · 음악]   ← 유지
```

### 잠금과의 관계

- **깨지 않음**: OutlineBox 홈피 골격 · 프로필 · 주간 · 게시판 · 음악  
- **바뀜**: 10자 전용 UI · 미니앨범 패널 → `DeskScene` 조립 위젯  
- 통짜 시안 이미지로 홈피 전체를 덮지 않음 (방법 2 레이어만)

### 친구 / 나

| | 나 | 친구 |
|--|----|------|
| DeskScene | 표시 · 편집 가능 | 표시 · 조회만 (날씨·업로드·뽀모도로 쓰기 비활성) |
| 메모 탭 | 오늘 10자 작성 | 오늘 10자 열람 (권한 있을 때) |
| 코르크 | 사진 배치/업로드 | 공개된 슬롯만 열람 |

(이전 옵션 A/B/C 후보는 **본 교체안으로 대체** — 단순 가산이 아니라 **해당 슬롯 치환**.)
...
```text
D0  본 기획 고정 — 10자+사진첩 슬롯 치환 ✅
D1  에셋 세트 v1 · DeskScene 조립 프로토타입
D2  DiaryHompyHome에서 Today I + Mini album 제거하고 DeskScene 삽입
D3  메모 ↔ tenCharText · 코르크 ↔ 실사진
D4  창문 변형 · 배지·램프 모션 · (선택) tomato
D5  친구 조회 모드 · 좌표 폴리시 · 주간 캘린더와 간격 조정
```

**S1**: 프로필·주간·게시판·(기존) 10자/앨범 실데이터 우선 가능.  
DeskScene 치환은 D1~D3으로 **홈피 CRUD와 맞물리게** 진행해도 되나, 에셋 없이 자리만 비우지 말 것.
...
1. `desk_bg`부터 개별 에셋 생성 (방법 2)  
2. `DeskScene` 프로토타입 → `DiaryHompyHome` 슬롯 치환  
3. S1 실데이터와 메모/코르크 바인딩  
