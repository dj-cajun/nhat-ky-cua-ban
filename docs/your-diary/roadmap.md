...
학교 레이어: [20](./20-school-trust-boundary-plan.md) — **B 완료**. **B.1 = staging JWT 실측 증명만 남음** ([21 sign-off](./21-phase-b1-staging-signoff.md)). 기획 논의 종료.
...
| **12·B.1** | staging JWT 실측 · 혼재 resolve · sign-off | ⏳ | [21](./21-phase-b1-staging-signoff.md) · `020` 체크리스트 |
