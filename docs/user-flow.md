...
## 1. Zalo 최초 진입 (온보딩 + 개척단)

```
[Zalo 미니앱 실행] → [Zalo 프로필 연동] → [학교·학급 선택] → [힌트 데이터 입력]
    → [개척단 관문: Zalo 3인 인증 + 퀴즈] → [메인 홈]
```
...
4. **학급이 아직 개설되지 않았으면** → 개척단 플로우 (Zalo 링크 3인 + 퀴즈 3개)
5. 개척 완료 또는 게이트 통과 → 내 다이어리 홈 (My Home)
